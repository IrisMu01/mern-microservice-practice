import {getDirectSurroundingCells, getSurroundingCells} from "../../../utils/utils";
import _ from "lodash";
import {mapValue} from "../../../utils/constants";

export const determineHumanActions = (state) => {
    ...
    const surroundingCells = getSurroundingCells(map, mapDimension, humanCoordinate.x, humanCoordinate.y);
    ...
    const directSurroundingCells = getDirectSurroundingCells(map, mapDimension, humanCoordinate.x, humanCoordinate.y);
    const waterNearby = _.filter(directSurroundingCells, cell => !fogMap[cell.x][cell.y] && cell.mapValue === mapValue.river);
    state.availableActions.human.buildBoat = waterNearby.length > 0
        && state.player.inventory.wood >= 5 && actionPointsAvailable;
    state.availableActions.human.fish = waterNearby.length > 0 && actionPointsAvailable;
...
export const determineDogActions = (state) => {
